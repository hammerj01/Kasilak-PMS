...
CREATE TABLE `tblbaptismal` (
  ...
  CONSTRAINT `FK_tblbaptismal_2` FOREIGN KEY (`BookID`) REFERENCES `tblbook` (`BookID`) ON UPDATE CASCADE,
...
INSERT INTO `tblperson` (`PersonID`,`Lastname`,`Middlename`,`Firstname`,`Birthdate`,`BirthPlace`,`Gender`,`Status`,`ClusterID`) VALUES 
 (1,'Merto','Acer','Hipolito','3/1/2011 10:20:44 AM','Cebu City',1,'',0),
 (2,'Sagisabal','Ricalde','Estrella','5/20/1982 11:59:49 AM','Corella, Bohol',0,'',0);
...
CREATE TABLE `tblsponsor` (
  ...
  `PersonID` int(10) unsigned NOT NULL,
  KEY `FK_tblsponsor_1` (`EventNo`),
  CONSTRAINT `FK_tblsponsor_1` FOREIGN KEY (`EventNo`) REFERENCES `tblbaptismal` (`BaptismalID`) ON UPDATE CASCADE,
  CONSTRAINT `FK_tblsponsor_2` FOREIGN KEY (`EventNo`) REFERENCES `tblmarriage` (`MarriageID`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
